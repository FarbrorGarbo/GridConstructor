...
export class SettingsDialog {
    ...
    public handleSettings = (key: string, value: number) => {
        if (this._settings) {
            const settingsCopy: Settings = {...this._settings};
            switch (key) {
...
export const SettingsDialogFC: React.FC<SettingsDialogFCProps> = (props) => {
    ...
    return (
        <div className={"dialog"} onMouseDown={e => e.stopPropagation()} onMouseMove={e => e.stopPropagation()}>
            ...
            <GenericNumberInput label={"Offset Horizontal"} min={0} max={999999} step={5} value={instance.settings.offsetH} returnValue={(val) => instance.handleSettings("offsetH", val)} />
